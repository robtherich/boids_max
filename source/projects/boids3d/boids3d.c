...
void Flock_bang(FlockPtr bflockPtr)
{
	...
	switch(bflockPtr->mode) { // newpos
		...
		for (i = 0; i < bflockPtr->numBoids; i++){
			...
			outlet_list(bflockPtr->out1, 0L, 4, out);
		}
		break;
		...
		for (i = 0; i < bflockPtr->numBoids; i++){						
			...
			azi = atan2(delta_z, delta_x) * bflockPtr->r2d;
			...
			SETFLOAT(out+8, azi);
			SETFLOAT(out+9, ele);
			outlet_list(bflockPtr->out1, 0L, 10, out);
		}
		break;	
	}
}
...
void Flock_dump(FlockPtr bflockPtr)
{
	...
	outlet_anything(bflockPtr->out2, gensym("flyrect"), 6, outList);
	...
	outlet_anything(bflockPtr->out2, gensym("attractpt"), 3, outList);
...
void Flock_set_pos(FlockPtr bflockPtr, Symbol *msg, short argc, Atom *argv)
{
	...
	if(argc) {
		...
		bflockPtr->boid[id].oldPos.z = temp[2];
		bflockPtr->boid[id].newPos.x = temp[0];
		bflockPtr->boid[id].newPos.y = temp[1];
		bflockPtr->boid[id].newPos.z = temp[2];
	}
}
...
void Flock_set_dir(FlockPtr bflockPtr, Symbol *msg, short argc, Atom *argv)
{
	...
	if(argc) {
		...
		bflockPtr->boid[id].oldDir.z = temp[2];
		bflockPtr->boid[id].newDir.x = temp[0];
		bflockPtr->boid[id].newDir.y = temp[1];
		bflockPtr->boid[id].newDir.z = temp[2];
